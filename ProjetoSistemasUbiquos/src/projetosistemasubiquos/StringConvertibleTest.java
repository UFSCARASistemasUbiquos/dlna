...
example.localservice.MyServiceWithStringConvertibles" style="include: INC1"/>
   * <p>
   * The state variables are all of UPnP datatype <code>string</code> because
   * Cling knows that the Java type of the annotated field is "string
convertible".
   * This is always the case for <code>java.net.URI</code> and
<code>java.net.URL</code>.
   * </p>
   * <p>
   * Any other Java type you'd like to use for automatic string conversion
has to be named
   * in the <code>@UpnpService</code> annotation on the class, like the
   * <code>MyStringConvertible</code>. Note that these types have to
   * have an appropriate <code>toString()</code> method and a single argument
constructor
   * that accepts a <code>java.lang.String</code> ("from string" conversion).
   * </p>
   * <p>
   * The <code>List&lt;Integer></code> is the collection you'd use in your
service
   * implementation to group several numbers. Let's assume that for UPnP
communication
   * you need a comma-separated representation of the individual values in a
string,
   * as is required by many of the UPnP A/V specifications. First, tell Cling
that
   * the state variable really is a string datatype, it can't infer that
   * from the field type. Then, if an action has this output argument,
instead of
   * manually creating the comma-separated string you pick the appropriate
converter
   * from the classes in <code>org.teleal.cling.model.types.csv.*</code> and
return
   * it from your action method. These are actually <code>java.util.List</
code>
   * implementations, so you could use them <em>instead</em> of
   * <code>java.util.List</code> if you don't care about the dependency. Any
action
...
Exception {
      ...
      public Object[][] getDevices() {
          ...
          try {
              return new LocalDevice[][]{
                      {createTestDevice
(MyServiceWithStringConvertibles.class)},
              };
          } catch (Exception ex) {
     ...
     public void validateBinding(LocalDevice device) {
         ...
         for (StateVariable stateVariable : svc.getStateVariables()) {
             assertEquals(stateVariable.getTypeDetails().getDatatype
().getBuiltin(), Datatype.Builtin.STRING);
         }

         assertEquals(svc.getActions().length, 9); // Has 8 actions plus
QueryStateVariableAction!

         assertEquals(svc.getAction("SetMyURL").getArguments().length, 1);
         assertEquals(svc.getAction("SetMyURL").getArguments()[0].getName(),
"In");
         assertEquals(svc.getAction("SetMyURL").getArguments()
[0].getDirection(), ActionArgument.Direction.IN);
         assertEquals(svc.getAction("SetMyURL").getArguments()
[0].getRelatedStateVariableName(), "MyURL");
     ...
     public void invokeActions(LocalDevice device) {
         ...
         ActionInvocation setMyURL = new ActionInvocation(svc.getAction
("SetMyURL"));
         ...
         ActionInvocation getMyURL = new ActionInvocation(svc.getAction
("GetMyURL"));
         ...
         ActionInvocation setMyURI = new ActionInvocation(svc.getAction
("SetMyURI"));
         ...
         ActionInvocation getMyURI = new ActionInvocation(svc.getAction
("GetMyURI"));
         ...
         ActionInvocation setMyNumbers = new ActionInvocation(svc.getAction
("SetMyNumbers"));
         ...
         ActionInvocation getMyNumbers = new ActionInvocation(svc.getAction
("GetMyNumbers"));
         ...
         ActionInvocation setMyStringConvertible = new ActionInvocation
(svc.getAction("SetMyStringConvertible"));
         setMyStringConvertible.setInput("In", "foobar");
         svc.getExecutor(setMyStringConvertible.getAction()).execute
(setMyStringConvertible);
         assertEquals(setMyStringConvertible.getFailure(), null);
         assertEquals(setMyStringConvertible.getOutput().length, 0);

         ActionInvocation getMyStringConvertible = new ActionInvocation
(svc.getAction("GetMyStringConvertible"));
         svc.getExecutor(getMyStringConvertible.getAction()).execute
(getMyStringConvertible);
         assertEquals(getMyStringConvertible.getFailure(), null);
         assertEquals(getMyStringConvertible.getOutput().length, 1);
         assertEquals(getMyStringConvertible.getOutput()[0].toString(),
"foobar");
