   ...
   * Working with enums
   * <p>
   * Java <code>enum</code>'s are special, unfortunately: You can't
instantiate
...
example.localservice.MyServiceWithEnum" style="include: INC1"/>
   * <p>
   * Cling will automatically assume that the datatype is a UPnP string if
the
   * field (or getter) or getter Java type is an enum. Furthermore, an
   * <code>&lt;allowedValueList&gt;</code> will be created in your service
descriptor
   * XML, so control points know that this state variable has in fact a
defined
...
Exception {
      ...
      public void validateBinding(LocalDevice device) {
          ...
          assertEquals(svc.getStateVariables()[0].getTypeDetails
().getDatatype().getBuiltin(), Datatype.Builtin.STRING);

          assertEquals(svc.getActions().length, 3); // Has 2 actions plus
QueryStateVariableAction!

          assertEquals(svc.getAction("GetColor").getArguments().length, 1);
          assertEquals(svc.getAction("GetColor").getArguments()[0].getName(),
"Out");
          assertEquals(svc.getAction("GetColor").getArguments()
[0].getDirection(), ActionArgument.Direction.OUT);
          assertEquals(svc.getAction("GetColor").getArguments()
[0].getRelatedStateVariableName(), "Color");

         assertEquals(svc.getAction("SetColor").getArguments().length, 1);
         assertEquals(svc.getAction("SetColor").getArguments()[0].getName(),
"In");
         assertEquals(svc.getAction("SetColor").getArguments()
...
[0].getRelatedStateVariableName(), "Color");
     ...
     public void invokeActions(LocalDevice device) {
         ...
         ActionInvocation setColor = new ActionInvocation(svc.getAction
("SetColor"));
         ...
         ActionInvocation getColor = new ActionInvocation(svc.getAction
("GetColor"));
         ...
         assertEquals(getColor.getOutput()[0].toString(),
MyServiceWithEnum.Color.Blue.name());
