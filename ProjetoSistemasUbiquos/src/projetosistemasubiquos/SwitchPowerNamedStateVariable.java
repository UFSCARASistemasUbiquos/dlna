   ...
   * <p>
   * If your mapped action method does not match the name of a mapped state
variable,
   ...
   * </p>
   * <p>
   * The "related statevariable" detection algorithm in Cling has one more
trick
   * up its sleeve however. The UPnP specification says that a state variable
which
   * is only ever used to describe the type of an input or output argument
should
   * be named with the prefix <code>A_ARG_TYPE_</code>. So if you do not name
the
   * related state variable of your action argument, Cling will also
   * look for a state variable with the name
   * <code>A_ARG_TYPE_[Name Of Your Argument]</code>. In the example above,
Cling
   ...
   * of state variables is already deprecated in UDA 1.0, there are <em>NO</
em>
   * state variables which are anything but type declarations for action
input/output
   * arguments. This is a good example why UPnP is such a horrid
specification.)
   * </p>
   */
  @UpnpService(
